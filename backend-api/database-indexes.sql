...
-- Run this after the initial database schema is created
-- =============================================

USE [switchlibrary-games];
GO
...
-- Covers: SearchGamesAsync (line 98-99 in SqlGameService.cs)
...
-- Alternative: Full-text index for game title and alternates (more efficient for text search)
-- Requires full-text search to be enabled on the database
-- Uncomment if you want to use full-text search instead of LIKE
/*
IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = OBJECT_ID('games_cache'))
BEGIN
    CREATE FULLTEXT INDEX ON games_cache(game_title, alternates)
    KEY INDEX PK_games_cache
    WITH STOPLIST = SYSTEM;
    PRINT 'Created full-text index on games_cache';
END
GO
*/
...
-- Covers: EnrichGamesWithRelatedData (line 645-650)
...
-- Covers: EnrichGamesWithRelatedData (line 653-659)
...
-- Covers: EnrichGamesWithRelatedData (line 662-668)
...
-- Covers: GetUpcomingGamesAsync (line 383-386)
...
-- Covers: GetRecommendationsAsync (line 500-501)
